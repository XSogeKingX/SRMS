...
class MainPanel extends JPanel
{
    ...
    private void createSearchFrame(String msg)
    {
        try
        {
            SearchFrame frame = new SearchFrame(msg,this);
    ...
    private void applicationInit()
    {
        try
        {
            ...
            con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","SCOTT","suman");
    ...
    private void populateTable(String qry)  //SELECT * FROM STUDENT WHERE ID = 1010 ORDER BY ID;
    {                                       //SELECT * FROM STUDENT WHERE NAME LIKE 'Su%' ORDER BY ID;
        try                                 //SELECT * FROM STUDENT WHERE COURSE = 'BTech' ORDER BY ID;
        {
    ...
    protected void prepareQuery(String data)
    {
        String qry = "";
        if(searchMode == 0)
        {
            int id = Integer.parseInt(data);
            qry = "SELECT ID,NAME,ADDRESS,PHONE,SEX,COURSE FROM STUDENT WHERE ID = "+id;
        }
        else if(searchMode == 1)
        {
            String nme = data;
            qry = "SELECT ID,NAME,ADDRESS,PHONE,SEX,COURSE FROM STUDENT WHERE NAME LIKE '"+nme+"%'";
        }
        else if(searchMode == 2)
        {
            String crs = data;
            qry = "SELECT ID,NAME,ADDRESS,PHONE,SEX,COURSE FROM STUDENT WHERE COURSE = '"+crs+"'";
        }
        tblModel.setRowCount(0);
        populateTable(qry);
    }
